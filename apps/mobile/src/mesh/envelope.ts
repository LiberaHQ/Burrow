...
import { base64ToBytes, bytesToBase64, utf8Decode, utf8Encode } from "../ble/base64";
...
export function encodeMsg(params: {
  ...
  const ciphertext = nacl.box(utf8Encode(params.text), nonce, params.toPublicKey, params.from.secretKey);
  const envelope: MsgEnvelope = {
    v: PROTOCOL_VERSION,
    type: "msg",
    from: params.from.hash,
    to: params.toHash,
    id: params.id ?? newMessageId(),
...
export function newMessageId(): string {
  return Array.from(nacl.randomBytes(8))
